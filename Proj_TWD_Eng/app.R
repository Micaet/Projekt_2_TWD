...
server <- function(input, output) {
  ...
  output$plot1 <- renderPlotly({
    ...
    plot <- ggplot(data, aes(x = Date, y = n)) +
      ...
      ggtitle(paste("Games per day       ", "             Total number of ggames:", total_games)) +  # Tytuł z odstępem tak dodałem spacjami to zostaw mnie w spokoju
      labs(
        x = "Date",
        y = "Number of games"
      ) +
  ...
  output$plot3 <- renderPlotly({
    ...
    plot3 <- ggplot(data, aes(x = Day, y = win_ratio)) +
      ...
      labs(
        title = paste("Winrate in each day"),
        x = "Day",
        y = "Winrate"
      ) +
      theme_minimal() +
